...
public class QoSCalculator implements QoSCalculatorIF {
	...
	public ReservationResults computeAllocation(
		...
		try{
			SplitPolicy splitPolicy = SplitPolicy.MAX_SPLIT;
			...
			res[0].setSplitPolicy(splitPolicy);
			//execution with p_ij=f_ij
			res[0] = ABGAP(k, requests, matrixF_ij, matrixU_ij, servPeriodsMap, thingsInfo, servNameThingsIdList, matrixM, epsilon, prio, splitPolicy, allocPolicy);
...
//			res[0].operationStatus = operationStatus;
			...
			res[1] = ABGAP(k, requests, matrixF_ij, matrixU_ij, servPeriodsMap, thingsInfo, servNameThingsIdList, matrixM, epsilon, prio, splitPolicy, allocPolicy);
...
//			}
	...
	private Reserveobj ABGAP(
			...
			AllocationPolicy allocPolicy) throws IOException,UnsupportedEncodingException,FileNotFoundException{
		...
		Statistics.printInputsABGAP(k, requests, matrixF_ij, matrixU_ij, servPeriodsMap, thingsInfo, servNameThingsIdList, matrixM, prio.name(), splitPolicy.name(), allocPolicy.name());
		
		res = GAP(k, requests, matrixF_ij, matrixU_ij, servPeriodsMap, thingsInfo, servNameThingsIdList, matrixM, teta, prio, splitPolicy);
		...
		res.setAllocPolicy(allocPolicy);
	...
	private Reserveobj GAP(
			...
			double teta,
			Priority prio,
			SplitPolicy splitPolicy) throws IOException, UnsupportedEncodingException,FileNotFoundException{
...
//}
	...
	private int chooseFactor(List<Integer> sj, SplitPolicy policy) {

		if(policy == SplitPolicy.MIN_SPLIT){
